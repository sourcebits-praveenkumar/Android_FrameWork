...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants
{
	...
	/**
	 * Return a new AQuery object that uses the found parent as a root. If no parent with matching
	 * id is found, operating view will be null and isExist() will return false.
	 * 
	 * 
	 * @param id
	 *            the parent id
	...
	/**
	 * Find the progress bar and show the progress for the next ajax/image request. Once ajax or
	 * image is called, current progress view is consumed. Subsequent ajax/image calls won't show
	 * progress view unless progress is called again.
	 * 
	 * If a file or network requests is required, the progress bar is set to be "VISIBLE". Once the
	 * requests completes, progress bar is set to "GONE".
	 * 
	 * @param id
	 ...
	 * 
	 * Once ajax or image is called, current progress view is consumed. Subsequent ajax/image calls
	 * won't show progress view unless progress is called again.
	 * 
	 * If a file or network requests is required, the progress bar is set to be "VISIBLE". Once the
	 * requests completes, progress bar is set to "GONE".
	 * 
	 ...
	 * 
	 * If a file or network requests is required, the dialog is shown. Once the requests completes,
	 * dialog is dismissed.
	 * 
	 * It's the caller responsibility to dismiss the dialog when the activity terminates before the
	 * ajax is completed. Calling aq.dismiss() in activity's onDestroy() will ensure all dialogs are
	 * properly dismissed.
	 * 
	 * @param dialog
	 * @return self
	...
	/**
	 * Set the text of a TextView with localized formatted string from application's package's
	 ...
	 * @param fallbackId
	 *            Fallback image if result is network fetch and image convert failed. 0 = no
	 *            fallback.
	 * @return self
	 * 
	 * @see testImage6
	 ...
	 *            fallback.
	 * @param preset
	 *            Default image to show before real image loaded. null = no preset.
	 * @param animId
	 *            Apply this animation when image is loaded. 0 = no animation. Also accept
	 *            AQuery.FADE_IN as a predefined 500ms fade in animation.
	 * @return self
	 * 
	 * @see testImage7
	 * 
	 ...
	 *            AQuery.FADE_IN as a predefined 500ms fade in animation.
	 * @param ratio
	 *            The desired aspect ratio of the imageview. Ratio is height / width, or
	 *            AQuery.RATIO_PRESERVE to preserve the original aspect ratio of the image.
	 * @return self
	 * 
	 * @see testImage12
	 * 
	 ...
	 * @param resId
	 *            Fallback image if result is network fetch and image convert failed.
	 * @param callback
	 *            Callback handler for setting the image.
	 * @return self
	 * 
	 ...
	 * @see testImage9
	 */
	public T image(String url, boolean memCache, boolean fileCache, int targetWidth, int resId, int round, BitmapAjaxCallback callback)
	{

		callback.targetWidth(targetWidth).fallback(resId).url(url).memCache(memCache).fileCache(fileCache).round(round);
	...
	/**
	 * Register a callback method for when the view is clicked. Method must have signature of
	 * method(View view).
	 * 
	 * @param handler
	...
	/**
	 * Register a callback method for when the view is long clicked. Method must have signature of
	 * method(View view).
	 * 
	 * @param handler
	 *            The handler that has the public callback method.
	...
	/**
	 * Register a callback method for when an item is clicked in the ListView. Method must have
	...
	/**
	 * Register a callback method for when an item is selected. Method must have signature of
	 * method(AdapterView<?> parent, View v, int pos, long id).
	 * 
	 * @param handler
	 *            The handler that has the public callback method.
	 * @param method
	...
	/**
	 * Register a callback method for when a list is scrolled to bottom. Method must have signature
	 * of method(AbsListView view, int scrollState).
	 * 
	 * @param handler
	 *            The handler that has the public callback method.
	 * @param method
	...
	/**
	 * Register a callback method for when a textview text is changed. Method must have signature of
	 * method(CharSequence s, int start, int before, int count)).
	 * 
	 * @param handler
	 *            The handler that has the public callback method.
	 * @param method
	...
	/**
	 * Advanced Ajax callback. User must manually prepare the callback object settings (url, type,
	 * etc...) by using its methods.
	 * 
	 * @param callback
	 *            callback handler
	 * @return self
	 ...
	 * 
	 * The expire param is the duration to consider cached data expired (if hit). For example,
	 * expire = 15 * 60 * 1000 means if the cache data is within 15 minutes old, return cached data
	 * immediately, otherwise go fetch the source again.
	 * 
	 * 
	 * @param url
	 *            url
	 ...
	 * Ajax call with various callback data types with file caching.
	 * 
	 * The expire param is the duration to consider cache data expired (if hit). For example, expire
	 * = 15 * 60 * 1000 means if the cache data is within 15 minutes old, return cached data
	 * immediately, otherwise go fetch the source again.
	 * 
	 * 
	 * @param url
	 ...
	 *            url to cache
	 * @param expire
	 *            duration in millseconds, 0 = never consider cached data as expired
	 * 
	 * @return self
	 * 
	 * @see testCache
	...
	/**
	 * @deprecated As of release 0.21.3, replaced by shouldDelay(int position, View convertView,
	 *             ViewGroup parent, String url)
	 * 
	 *             This method is less efficicent and promote file check which could hold up the UI
	 *             thread.
	 * 
	 ...
	 * 
	 * Designed to be used inside getGroupView(int groupPosition, boolean isExpanded, View
	 * convertView, ViewGroup parent) of an expandable list adapter.
	 * 
	 * @param groupPosition
	 ...
	 * 
	 * Designed to be used inside getChildView(int groupPosition, int childPosition, boolean
	 * isLastChild, View convertView, ViewGroup parent) of an expandable list adapter.
	 * 
	 * @param groupPosition
	 *            the group position of the item
	 * @param childPosition
	...
	/**
	 * Determines if a list or gallery view item should delay loading a url resource because the
	 * view is scrolling very fast. The primary purpose of this method is to skip loading remote
	 * resources (such as images) over the internet until the list stop flinging and the user is
	 * focusing on the displaying items.
	 * 
	 * If the scrolling stops and there are delayed items displaying, the getView method will be
	 * called again to force the delayed items to be redrawn. During redraw, this method will always
	 * return false, thus allowing a particular resource to be loaded.
	 * 
	 * Designed to be used inside getView(int position, View convertView, ViewGroup parent) of an
	 * adapter.
	 * 
	 * 
	 * <br>
	 ...
	 * 
	 * <br>
	 * This method uses the setOnScrollListener() method and will override any previously non-aquery
	 * assigned scroll listener. If a scrolled listener is required, use the aquery method
	 * scrolled(OnScrollListener listener) to set the listener instead of directly calling
	 * setOnScrollListener().
	 * 
	 * @param position
	...
	/**
	 * Create a temporary file on EXTERNAL storage (sdcard) that holds the cached content of the
	 * url. Returns null if url is not cached, or the system cannot create such file (sdcard is
	 * absent, such as in emulator).
	 * 
	 * The returned file is accessable to all apps, therefore it is ideal for sharing content (such
	 * as photo) via the intent mechanism.
	 * 
	 * <br>
	 * <br>
	 * Example Usage:
	 ...
	 * 
	 * <br>
	 * The temp file will be deleted when AQUtility.cleanCacheAsync is invoked, or the file can be
	 * explicitly deleted after use.
	 * 
	 * @param url
	 *            The url of the desired cached content.
	 * @param filename
	 *            The desired file name, which might be used by other apps to describe the content,
	 ...
	 *            Id of the desired animation.
	 * @param listener
	 *            The listener to recieve notifications from the animation on its events.
	 * @return self
	 * 
	 * 
	 */
	 ...
	 * Load the webview with an image with a url.
	 * 
	 * Zoom control can only be disabled for API level 14+. Lower level apis will always show zoom
	 * control if zoom is enabled.
	 * 
	 * Note that color is AARRGGBB, where AA is the alpha value. Solid color (0xFF??????) should be
	 * used for performance reason. Transparent background is not well supported by WebView in most
	 * devices.
	 * 
	 * @param url
	 *            The image url
	 ...
	 * 
	 * This method is similar to LayoutInflater.inflate() but with sanity checks against the layout
	 * type of the convert view.
	 * 
